...
class CreateOrdersController {
    async handler(request: Request, response: Response): Promise<Response>{
        ...
        const { customerName, items, status, paymentType, amount } = request.body;
        const createOrdersUseCase = container.resolve(CreateOrdersUseCase);
        const orderId = await createOrdersUseCase.execute({ customerName, items, status, paymentType, amount, created_by: user.id });
